...
def main():
    ...
    try:
        ...
        _esfera_html = {}
    # Election Threat Landscape (opción B): registro dirigido por datos
    # (data/elecciones.yaml). Fases EEAS + cobertura + actor + 5D. Descriptivo.
    # Se muestra EN el tab del tema `elecciones` (su sitio temático), no en
    # Transparencia. Se computa aquí (antes del bucle de panes) para inyectarlo.
    _elec_tema_html = {}
    try:
  ...
  (function(){{
    ...
    _val_euvs_domains = 1311
    _val_euvs_cases = 10682
    _val_euvs_spanish = 243
    _val_euvs_langs = 42
    _val_dom_found = 0
    _val_ev_found = 0
    _val_ari_score = "1,000"
    _validacion_html = f"""
...
<td style="padding:6px 10px;border-bottom:1px solid #e2e8f0">Dominios rusos/chinos (2015-23) fuera de <b>scope actual</b>. RT ES capturado ({_val_ev_found} ev) sin amplificación.</td>
</tr>
<tr>
<td style="padding:6px 10px;border-bottom:1px solid #e2e8f0">📊 <b>Dominios encontrados</b></td>
<td style="text-align:center;padding:6px 10px;border-bottom:1px solid #e2e8f0"><span style="color:#2563eb;font-weight:700">{_val_dom_found} de {_val_euvs_domains}</span></td>
<td style="padding:6px 10px;border-bottom:1px solid #e2e8f0">Dominios EUvsDisinfo en <b>cualquier evento</b> del corpus. Sin señal en clusters.</td>
</tr>
</table>
...
Los <b>{_val_euvs_spanish}</b> casos en castellano (de {_val_euvs_cases} totales)
son el terreno que cubriremos al ampliar el catálogo.
<br><span style="font-size:.78rem;color:#78350f">Dataset EUvsDisinfo · {_val_euvs_langs} idiomas.</span>
</div>
</div>
"""
...
if ('serviceWorker' in navigator) {{
  navigator.serviceWorker.register('/sw.js', {{updateViaCache:'none'}}).then(function(r){{ r.update(); }}).catch(function(e){{ console.warn('SW no registrado', e); }});
}}
</script>
<script>
(function(){{
